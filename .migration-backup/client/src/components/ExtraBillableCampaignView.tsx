...
function CrewDialog({
  ...
  const [memberIds, setMemberIds] = useState<Set<string>>(
    new Set(crew?.members.map(m => m.userId) ?? []),
  );
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const existing = new Set(crew?.members.map(m => m.userId) ?? [saved.leaderUserId]);
      const desired = new Set(memberIds);
      desired.add(leaderUserId);
      const toAdd: string[] = [];
      const toRemove: string[] = [];
      desired.forEach(id => { if (!existing.has(id)) toAdd.push(id); });
      existing.forEach(id => { if (!desired.has(id) && id !== leaderUserId) toRemove.push(id); });
