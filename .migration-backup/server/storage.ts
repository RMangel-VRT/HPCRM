...
export interface IStorage {
  ...
  getCampaigns(companyId: string, assignedToId?: string, additionalCampaignIds?: string[]): Promise<CampaignWithProgress[]>;
...
export class PgStorage implements IStorage {
  ...
  async getCampaigns(companyId: string, assignedToId?: string, additionalCampaignIds?: string[]): Promise<CampaignWithProgress[]> {
    let whereClause;
    if (assignedToId) {
      const orConditions = [
        eq(campaigns.assignedToId, assignedToId),
        eq(campaigns.assignedToId2, assignedToId),
      ];
      if (additionalCampaignIds && additionalCampaignIds.length > 0) {
        orConditions.push(inArray(campaigns.id, additionalCampaignIds));
      }
      whereClause = and(eq(campaigns.companyId, companyId), or(...orConditions));
    } else {
      whereClause = eq(campaigns.companyId, companyId);
    }
