...
// Startup migration: Remove deprecated "Execution Task" ticket type by migrating tickets to "To-Do"
// This is a one-time migration that runs on server startup
export async function migrateExecutionTaskToToDo(): Promise<void> {
  console.log("Running startup migration: Removing deprecated Execution Task ticket type...");
  ...
  try {
    ...
    for (const company of companies) {
      // Find Execution Task ticket type for this company
      const ticketTypesForCompany = await storage.getTicketTypes(company.id);
      const executionTask = ticketTypesForCompany.find(tt => tt.name === "Execution Task");
      
      if (!executionTask) {
        continue; // No Execution Task in this company
      }
      
      // Find To-Do ticket type for this company
      const toDoType = ticketTypesForCompany.find(tt => tt.name === "To-Do");
      
      if (!toDoType) {
        console.log(`Skipping company ${company.id}: No To-Do ticket type found`);
        continue;
      }
      
      // Get first status of To-Do workflow (by display_order)
      const toDoStatuses = await storage.getTicketTypeStatuses(toDoType.id);
      const sortedToDoStatuses = toDoStatuses.sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0));
      
      if (sortedToDoStatuses.length === 0) {
        console.log(`Skipping company ${company.id}: To-Do has no statuses`);
        continue;
      }
      
      const firstToDoStatus = sortedToDoStatuses[0];
      
      // Find tickets on Execution Task
      const allTickets = await storage.getTickets(company.id);
      const ticketsToMigrate = allTickets.filter(t => t.ticketTypeId === executionTask.id);
      
      if (ticketsToMigrate.length > 0) {
        // Migrate tickets to To-Do with first status
        for (const ticket of ticketsToMigrate) {
          await storage.updateTicket(ticket.id, company.id, {
            ticketTypeId: toDoType.id,
            currentStatusId: firstToDoStatus.id
          });
        }
        console.log(`Migrated ${ticketsToMigrate.length} tickets from Execution Task to To-Do`);
      }
      
      // Delete Execution Task statuses first (foreign key constraint)
      const executionTaskStatuses = await storage.getTicketTypeStatuses(executionTask.id);
      for (const status of executionTaskStatuses) {
        await storage.deleteTicketTypeStatus(status.id);
      }
      
      // Delete Execution Task ticket type
      await storage.deleteTicketType(executionTask.id, company.id);
      
      console.log(`Deleted Execution Task ticket type for company ${company.id}`);
    }
    
    console.log("Startup migration complete: Execution Task cleanup finished");
  } catch (error) {
    console.error("Error during Execution Task migration:", error);
  }
}
