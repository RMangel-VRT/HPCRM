...
import type { Communication, Customer, CommunicationWithDetails, CommunicationTemplate, CommunicationAuditLogWithUser, CommunicationAutomationRule } from "@shared/schema";
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger data-testid="select-default-compose-type">
                            <SelectValue placeholder="Use template type" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="none">Use template type</SelectItem>
                          <SelectItem value="email">Email</SelectItem>
                          <SelectItem value="sms">SMS</SelectItem>
                          <SelectItem value="note">Note</SelectItem>
                          <SelectItem value="letter">Letter</SelectItem>
...
export default function AllCommunicationsTab() {
  const { toast } = useToast();
  const permissions = useCommPermissions();
  const { search: shellSearch, viewAs, fromDate, toDate } = useCommunicationsShell();
  const searchString = useSearch();

  const [sectionFilter, setSectionFilter] = useState<SectionFilter>(() => resolveViewParam(new URLSearchParams(searchString).get("view") ?? undefined));
  ...
  const filteredCommunications = useMemo(() => {
    ...
    if (shellSearch.trim()) {
      const q = shellSearch.toLowerCase();
      items = items.filter(
        (c) =>
    ...
    return items;
  }, [communications, sectionFilter, typeFilter, statusFilter, customerFilter, shellSearch]);

  const navSections: { id: SectionFilter; label: string; icon: React.ElementType; count?: number; requiresAdmin?: boolean }[] = [
    ...
    <div className="flex flex-1 min-h-0 overflow-hidden">
      <div className="w-52 shrink-0 border-r bg-muted/30 flex flex-col overflow-y-auto">
        <div className="p-4 border-b space-y-2">
          <div>
            <h1 className="text-sm font-semibold text-foreground">All Communications</h1>
