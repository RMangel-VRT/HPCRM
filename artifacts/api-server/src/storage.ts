...
export interface IStorage {
  ...
  getMyExtraBillableBatches(userId: string, role: string, companyId: string): Promise<MyExtraBillableBatch[]>;
  ...
  // Mailbox Backfill Runs
  createMailboxBackfillRun(data: InsertMailboxBackfillRun): Promise<MailboxBackfillRun>;
  updateMailboxBackfillRun(id: string, updates: Partial<InsertMailboxBackfillRun>): Promise<MailboxBackfillRun | undefined>;
...
export class PgStorage implements IStorage {
  ...
  private async _getVsSheetMap(vsIds: string[]): Promise<Record<string, import("@workspace/db").VisualScopeSheetWithCustomer>> {
  ...
  async getProposals(companyId: string): Promise<ProposalWithDetails[]> {
    ...
    const files = proposalIds.length > 0
      ...
      : [];
    const plantItemsAll = proposalIds.length > 0
      ? await db.select().from(proposalPlantItems).where(inArray(proposalPlantItems.proposalId, proposalIds)).orderBy(proposalPlantItems.displayOrder)
      : [];
    ...
    return rows.map(r => ({
      ...
      plantItems: plantItemsAll.filter(p => p.proposalId === r.proposal.id),
  ...
  async getProposalById(id: string, companyId: string): Promise<ProposalWithDetails | undefined> {
    ...
    const plantItems = await db.select().from(proposalPlantItems)
      .where(and(eq(proposalPlantItems.proposalId, id), eq(proposalPlantItems.companyId, companyId)))
      .orderBy(proposalPlantItems.displayOrder);
    ...
    const ppId = rows[0].proposal.plantPaletteId;
    let plantPalette: (typeof plantPalettes.$inferSelect & { items: (typeof plantPaletteItems.$inferSelect)[] }) | null = null;
    if (ppId) {
      const [paletteRow] = await db.select().from(plantPalettes)
        .where(and(eq(plantPalettes.id, ppId), eq(plantPalettes.companyId, companyId)));
      if (paletteRow) {
        const items = await db.select().from(plantPaletteItems)
          .where(and(eq(plantPaletteItems.paletteId, ppId), eq(plantPaletteItems.companyId, companyId)))
          .orderBy(plantPaletteItems.displayOrder);
        plantPalette = { ...paletteRow, items };
      }
    }
  ...
  async getMyExtraBillableBatches(userId: string, role: string, companyId: string): Promise<MyExtraBillableBatch[]> {
    const isAdminOrOffice = role === "admin" || role === "office";
    const baseSelect = {
      crewId: campaignCrews.id,
      crewName: campaignCrews.name,
      crewColor: campaignCrews.color,
      leaderUserId: campaignCrews.leaderUserId,
    ...
    if (isAdminOrOffice) {
      crewRows = await db
        .select(baseSelect)
        .from(campaignCrews)
        .innerJoin(campaigns, eq(campaignCrews.campaignId, campaigns.id))
        .innerJoin(users, eq(campaignCrews.leaderUserId, users.id))
        .where(and(
          eq(campaignCrews.companyId, companyId),
          eq(campaigns.category, "extra_billable"),
        ))
        .orderBy(desc(campaigns.windowEnd), asc(campaignCrews.name));
    } else {
      crewRows = await db
        ...
        .innerJoin(campaigns, eq(campaignCrews.campaignId, campaigns.id))
        .innerJoin(users, eq(campaignCrews.leaderUserId, users.id))
        .leftJoin(campaignCrewMembers, eq(campaignCrewMembers.campaignCrewId, campaignCrews.id))
        .where(and(
          eq(campaignCrews.companyId, companyId),
          eq(campaigns.category, "extra_billable"),
          or(eq(campaignCrews.leaderUserId, userId), eq(campaignCrewMembers.userId, userId)),
        ));
    }

    if (crewRows.length === 0) return [];
    const crewIds = crewRows.map(c => c.crewId);
    ...
    const itemAggRows = await db
      ...
      .from(campaignItems)
      .where(and(
        eq(campaignItems.companyId, companyId),
        inArray(campaignItems.assignedCampaignCrewId, crewIds),
      ))
      .groupBy(campaignItems.assignedCampaignCrewId);

    const aggByCrew = new Map<string, { total: number; completed: number; pending: number; photos: number; nextDueDate: string | null }>();
    for (const r of itemAggRows) {
      if (r.crewId) {
        aggByCrew.set(r.crewId, {
    ...
    return crewRows.map(c => {
      const agg = aggByCrew.get(c.crewId);
      return {
        ...
        crewId: c.crewId,
        crewName: c.crewName,
        crewColor: c.crewColor,
        leaderUserId: c.leaderUserId,
        leaderName: c.leaderName,
        isLeader: c.leaderUserId === userId,
        assignedItemCount: agg?.total ?? 0,
        completedItemCount: agg?.completed ?? 0,
        pendingItemCount: agg?.pending ?? 0,
        photoCount: agg?.photos ?? 0,
  ...
  async createMailboxBackfillRun(data: InsertMailboxBackfillRun): Promise<MailboxBackfillRun> {
    const [row] = await db.insert(mailboxBackfillRuns).values(data as typeof mailboxBackfillRuns.$inferInsert).returning();
    return row;
  }
  ...
  async getActiveMailboxBackfillRun(mailboxAccountId: string): Promise<MailboxBackfillRun | undefined> {
    const [row] = await db
      .select()
      .from(mailboxBackfillRuns)
      .where(
        and(
          eq(mailboxBackfillRuns.mailboxAccountId, mailboxAccountId),
