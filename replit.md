...
#### Campaign System
A batch property checklist system for organizing work across multiple properties within a completion window. Admin/Office users create campaigns, select properties, and assign tasks. Field workers can mark items complete, add notes, upload photos, or skip with a reason. Campaigns auto-complete and support archive/reactivate, search/filter, and progress tracking. Supports two categories: General and Chemical. Chemical campaigns include a 3-step communication workflow (Pre-Work Communication, Work Completion, Post-Completion Communication) with automated email notifications via SendGrid to customer primary contacts at pre-work and post-completion steps. Chemical campaign items show step indicators in the campaign detail list and a full workflow stepper on the item detail page. Role-based permissions control who can send communications (admin, office, chemical_manager) vs. complete work (all field roles).
