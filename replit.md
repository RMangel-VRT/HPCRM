...
- Keep `lib/db/drizzle.config.ts` as the only active Drizzle config in the repository. `.migration-backup/` may contain the checked-in SQL files under `migrations/`, but never another `drizzle.config.*`; Replit Publish can discover the stale config and generate destructive drops from its obsolete schema.
