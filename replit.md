...
P7 (Ticket Integration): Proposals link to tickets via `ticketId` FK on the proposals table. Project and Estimate Request tickets show a "Proposals" card on the Overview tab (admin/office only) listing linked proposals with version/status badges. A "New Proposal" button auto-creates a proposal linked to the ticket and navigates to the draft editor. The ProposalDraft page shows a "← Ticket: [title]" breadcrumb when the proposal has a linked ticket. Backend: `GET /api/tickets/:ticketId/proposals` returns linked proposals; `POST /api/proposals` now accepts optional `ticketId`.
