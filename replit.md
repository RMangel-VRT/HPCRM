...
The backend uses Express.js and TypeScript. Authentication is handled by Passport.js with session-based management stored in PostgreSQL. It supports multi-tenancy with granular role-based access (Admin, Office, Field Manager, Chemical Manager, Field, Irrigation Manager, Shop Manager). Data is persisted in PostgreSQL (Neon serverless) using Drizzle ORM and Drizzle Kit for migrations. Replit's object storage (Google Cloud Storage) stores contract PDF documents and visual scope images with company-scoped ACL and presigned URL uploads.

#### Internationalization (i18n)
Full Spanish/English language support using i18next and react-i18next. Each user has a `language` preference column (`en` or `es`) stored in the users table. The language syncs on login via `useEffect` in App.tsx. A globe toggle button in the sidebar footer switches language instantly without page reload via `PATCH /api/auth/language`. Translation files are in `client/src/i18n/locales/en.ts` and `es.ts`, covering all ~50 pages, sidebar navigation, dialogs, toasts, form labels, and status strings. New users can be created with a language preference via the Team page. The i18n setup is in `client/src/i18n/index.ts`.
...
VS5 adds Finalize-Grade Freezing of Visual Scope Snapshots. When a proposal with an attached VS sheet is finalized, the required VS PNGs (always combined; base-only if flag set; overlay-only if flag set) are rendered once via the VS3 renderer, stored permanently in GCS under `proposal-versions/{proposalId}/v{N}/visual-scope-{type}.png`, and their paths + metadata (title, date, width, flags, frozenAt) are stored on the `proposal_versions` record. Frozen assets are served from a dedicated endpoint: GET `/api/proposals/:id/versions/:versionId/visual-scope/:type` (combined|base|overlay) — never from live rendering. ProposalVersion page shows a "Visual Scope Snapshot (Frozen)" card when `vsCombinedPath` is set, with preview/download buttons for all frozen types. Fail-safe: if VS render or GCS upload fails, the entire finalization aborts (no partial version record). Schema: 10 new columns on `proposal_versions` — `visual_scope_sheet_id`, `visual_scope_title`, `visual_scope_date`, `vs_export_width`, `vs_included_base`, `vs_included_overlay`, `vs_frozen_at`, `vs_combined_path`, `vs_base_path`, `vs_overlay_path`. Bug fix: `visualScopeRenderer.ts` now uses `downloadByPath()` (not `getObjectEntityFile()`) to load GCS private-storage base images, fixing VS rendering for captured high-res images.
